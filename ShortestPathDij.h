...
void DisplayPathAndDist(const AdjListDirNetwork<ElemType, WeightType> &g, int v0, int *path, WeightType *dist,int s,int t)
{
    ElemType e1, e2;
	...
	for (int v1 = 0; v1 < g.GetVexNum(); v1++)	{
       ...
       if (v1 != v0) 
	      if (dist[v1] < infinity )		{	// 存在从v0到v1最短路径
	      	 cout << "the least distance from " << e1 << " to " << e2 << " is " << dist[v1] << endl; 
			 //cout << "从顶点 " << e1 << " 到 " << e2 << ":";
			 //cout << " 路径长度 = " << dist[v1];
			 cout << "the path is:";
...
// 用Dijkstra算法求有向网g从顶点v0到其余顶点v的最短路径path和路径长度dist[v],
